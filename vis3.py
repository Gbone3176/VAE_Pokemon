...
def slerp(v1, v2, t):
    ...
    Args:
        v1: First vector (torch.Tensor)
        v2: Second vector (torch.Tensor)
    ...
    dot_product = torch.clamp(torch.sum(v1_norm * v2_norm), -1.0, 1.0)
    theta = torch.acos(dot_product)
    
    # Handle small angles (fall back to linear interpolation)
...
with torch.no_grad():
    ...
    save_image_tensor(img_tl, f"Top-Left (idx {img_indices_4[0]})", f"vis/exp3-SlerpInterpolation/corner_tl_{img_indices_4[0]}.png")
    save_image_tensor(img_tr, f"Top-Right (idx {img_indices_4[1]})", f"vis/exp3-SlerpInterpolation/corner_tr_{img_indices_4[1]}.png")
    save_image_tensor(img_bl, f"Bottom-Left (idx {img_indices_4[2]})", f"vis/exp3-SlerpInterpolation/corner_bl_{img_indices_4[2]}.png")
    save_image_tensor(img_br, f"Bottom-Right (idx {img_indices_4[3]})", f"vis/exp3-SlerpInterpolation/corner_br_{img_indices_4[3]}.png")
    
    # ===Four-corner spherical interpolation in latent space===
    ...
    # Create 2D spherical interpolation grid
    interp_latents_grid = []
    for i in range(grid_steps):
        for j in range(grid_steps):
            ...
            # Spherical interpolation:
            # top edge: slerp between top-left and top-right
            lat_top = slerp(mu_tl, mu_tr, alpha_j)
            # bottom edge: slerp between bottom-left and bottom-right
            lat_bottom = slerp(mu_bl, mu_br, alpha_j)
            # final: slerp between top and bottom
            lat = slerp(lat_top, lat_bottom, alpha_i)
    ...
    grid_file = f"vis/exp3-SlerpInterpolation/4img_spherical_interp_{file_name}_idx{img_indices_4}_steps{grid_steps}.png"
    plt.savefig(grid_file)
    plt.close()
    
    print(f"Spherical interpolation grid saved to: {grid_file}")
